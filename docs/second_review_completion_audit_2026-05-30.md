...
| Confirm/implement dedicated-server entity binding client prediction | `EntityBindingSyncPayload` serializes entity bindings and `InteractionSettings.Snapshot`; `EntityTargetSyncPayload` serializes nearby server-matched registered entity UUIDs/ranges; payloads are registered in `ModPackets`; `InteractionSyncService` sends definitions on data-pack sync/reload and target snapshots every 20 ticks; `ClientInteractionNetworking` applies them and clears synced state on join/disconnect. | Complete |
...
| Full client GUI manual test from second review P0 | `docs/manual_client_test_result_2026-05-30_second.md` contains the hand-test checklist, but result log is still `Pending human GUI execution`. The first actual Windows GUI check found the tag-based prediction failure now documented in `manual_client_test_result_2026-05-30_second.md`; post-`EntityTargetSyncPayload` retest evidence is still pending. | Incomplete / externally blocked |
